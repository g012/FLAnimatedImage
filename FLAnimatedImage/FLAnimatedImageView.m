...
- (void)startAnimating
{
    if (self.animatedImage) {
        ...
        self.displayLink.preferredFramesPerSecond = 1.0 / MAX([self frameDelayGreatestCommonDivisor], 1e-6);
...
- (void)displayDidRefresh:(CADisplayLink *)displayLink
{
    ...
    if (delayTimeNumber) {
        ...
        if (image) {
            ...
            self.accumulator += displayLink.duration * (60.0 / MAX(displayLink.preferredFramesPerSecond, 1e-6));
            ...
            if ([self.debug_delegate respondsToSelector:@selector(debug_animatedImageView:waitingForFrame:duration:)]) {
                ...
                [self.debug_delegate debug_animatedImageView:self waitingForFrame:self.currentFrameIndex duration:(NSTimeInterval)displayLink.duration * (60.0 / MAX(displayLink.preferredFramesPerSecond, 1e-6))];
            }
#endif
        }
    } else {
